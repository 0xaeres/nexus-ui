...
export function PageHeader({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div className="border-b border-border bg-surface-glass/85 backdrop-blur-xl">
      <div
        className={cn(
...
export function PageBody({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div className="flex-1 overflow-auto">
      <div className={cn('mx-auto w-full py-8 space-y-8 lg:py-10', PAGE_PADDING_X, className)}>
        {children}
      </div>
    </div>
  )
